...
mpiPlatform = cliParams.platform
if mpiPlatform=="":
   mpiPlatform=PlatformModule.getPlatformString()
print mpiPlatform
os.listdir('.')
...
if (mpiPlatform=="Linux_i686_glibc-2.3.4" or mpiPlatform=="Linux_x86_64_glibc-2.3.4" or mpiPlatform=="slc4_amd64_gcc34" or mpiPlatform=="slc4_ia32_gcc34"):
   ...
   mpiVersion  = "mpich.%s.tar.gz" % (mpiPlatform)
else:
   mpi2Version = "MPICH2.tar.gz"
   mpiVersion  = "MPICH1.tar.gz"

mpi2Version = "MPICH2.tar.gz"
mpiVersion  = "MPICH1.tar.gz"
os.chdir(".")
cmd = ("wget -np http://dirac.eela.if.ufrj.br/MPI/%s")%(mpi2Version)
r=commands.getoutput(cmd)
cmd = ("wget -np http://dirac.eela.if.ufrj.br/MPI/%s")%(mpiVersion)
r=commands.getoutput(cmd)
cmd = ("wget -np http://dirac.eela.if.ufrj.br/MPI/mpd.conf")
os.system(cmd)
os.listdir('.')
cmd = "tar -xvzf MPICH2.tar.gz"
r=commands.getoutput(cmd)
os.listdir('.')
cmd = "tar -xvzf MPICH1.tar.gz"
r=commands.getoutput(cmd)
os.listdir('.')
cmd = ("wget -np http://dirac.eela.if.ufrj.br/MPI/MPIAgent.py")
r=commands.getoutput(cmd)
os.listdir('.')
...
os.chdir ("..")
### INSTALLATION MPICH1
if mpiVersion == "MPICH1.tar.gz":
  dir1=os.path.join((commands.getoutput('pwd')),('mpich-1.2.7p1'))
  dir = str(dir1)
  os.chdir ("./mpich-1.2.7p1/")
  installMPI = (('./configure --prefix=%s --exec-prefix=%s;make;make install') % (dirInstallMPICH1, dirInstallMPICH1))
  commands.getoutput(installMPI)
else:
  dir1=os.path.join((commands.getoutput('pwd')),('mpich'))
  dir = str(dir1)
  os.chdir ("./mpich/")
  installMPI = ('ls -la')
  commands.getoutput(installMPI)
...
# Start MPIJobAgent
#
print "VH >>>>>", jobAgentOpts, inProcessOpts,  extraCFG
####mpiAgent = 'dirac-agent WorkloadManagement/MPIAgent %s %s %s' % (
####            jobAgentOpts, inProcessOpts,  extraCFG )
####logINFO( "MPIAgent execution command:\n%s" % mpiAgent )

logINFO( 'Starting MPIJobAgent' )

os.environ['PYTHONUNBUFFERED'] = 'yes'

startTime = time.time()
diracAgentScript = os.path.join( rootPath, "scripts", "dirac-agent" )
mpiJobAgent = '%s WorkloadManagement/MPIAgent %s %s %s' % ( diracAgentScript,
                                                         " ".join( jobAgentOpts ),
                                                         " ".join( inProcessOpts ),
                                                         " ".join( extraCFG ) )

logINFO( "MPIJobAgent execution command:\n%s" % mpiJobAgent )

if not cliParams.dryRun:
  os.system( mpiJobAgent )
